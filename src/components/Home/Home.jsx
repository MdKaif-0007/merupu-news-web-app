...
const Home = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row px-4 lg:px-10 py-6 gap-4 overflow-hidden">
      {/* Left Ad Box - Only show if ad loads */}
      {leftAdLoaded && (
        <aside className="hidden lg:block w-full lg:w-1/5 h-[800px] rounded flex justify-center sticky top-4 overflow-hidden">
          <GoogleAd onLoad={() => setLeftAdLoaded(true)} onError={() => setLeftAdLoaded(false)} />
        </aside>
      )}
      
      {/* Main News Content */}
      <main className={`w-full mx-auto ${leftAdLoaded && rightAdLoaded ? 'lg:flex-1' : 'lg:w-7/12'} space-y-8`}>
        <NewsSection category="latest-news" />
      </main>

      {/* Right Ad Box - Only show if ad loads */}
      {rightAdLoaded && (
        <aside className="hidden lg:block w-full lg:w-1/5 h-[800px] rounded flex justify-center sticky top-4 overflow-hidden">
          <GoogleAd onLoad={() => setRightAdLoaded(true)} onError={() => setRightAdLoaded(false)} />
        </aside>
      )}
    </div>
  );
